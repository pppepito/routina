"use client"

import { useState, useEffect, useCallback } from "react"
import type { Habit, HabitCompletion, HabitWithProgress, CompletionStatus } from "../types/habit"
import { CapacitorStorage } from "../lib/capacitor-storage"

// Create a unique EventTarget to broadcast changes across components
const habitEventTarget = new EventTarget()
const HABIT_STORAGE_EVENT = "habit-storage-change"

// Helper function to dispatch the custom event
const dispatchStorageChangeEvent = () => {
  habitEventTarget.dispatchEvent(new Event(HABIT_STORAGE_EVENT))
}

export function useHabits() {
  const [habits, setHabits] = useState<Habit[]>([])
  const [completions, setCompletions] = useState<HabitCompletion[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [celebratingHabits, setCelebratingHabits] = useState<Set<string>>(new Set())
  const [weekOffset, setWeekOffset] = useState(0)
  const [syncStatus, setSyncStatus] = useState<"synced" | "pending" | "offline">("synced")

  // Function to load data from storage with enhanced error handling
  const loadData = useCallback(async () => {
    try {
      setIsLoading(true)

      // Try multiple storage methods for maximum reliability
      const [savedHabits, savedCompletions] = await Promise.all([
        CapacitorStorage.getItem("habits").catch(() => localStorage.getItem("habits")),
        CapacitorStorage.getItem("completions").catch(() => localStorage.getItem("completions")),
      ])

      if (savedHabits) {
        const parsedHabits = JSON.parse(savedHabits)
        const habitsWithDates = parsedHabits.map((habit: any) => ({
          ...habit,
          createdAt: new Date(habit.createdAt),
        }))
        setHabits(habitsWithDates)
      } else {
        setHabits([])
      }

      if (savedCompletions) {
        const parsedCompletions = JSON.parse(savedCompletions)
        setCompletions(parsedCompletions)
      } else {
        setCompletions([])
      }

      // Check if we need to sync with cloud
      await checkCloudSync()
    } catch (error) {
      console.error("Error loading data from storage:", error)
      setHabits([])
      setCompletions([])
    } finally {
      setIsLoading(false)
    }
  }, [])

  // Enhanced cloud sync check
  const checkCloudSync = async () => {
    try {
      // Check if user is authenticated and online
      const isOnline = navigator.onLine
      const hasCloudData = await CapacitorStorage.getItem("cloud-sync-enabled")

      if (isOnline && hasCloudData === "true") {
        setSyncStatus("synced")
        // Trigger cloud sync if available
        const cloudSyncEvent = new CustomEvent("trigger-cloud-sync")
        window.dispatchEvent(cloudSyncEvent)
      } else if (!isOnline) {
        setSyncStatus("offline")
      } else {
        setSyncStatus("pending")
      }
    } catch (error) {
      console.error("Cloud sync check failed:", error)
      setSyncStatus("offline")
    }
  }

  // Enhanced save function with multiple backup strategies
  const saveDataSafely = async (key: string, data: any) => {
    try {
      const jsonData = JSON.stringify(data)

      // Primary storage (Capacitor)
      await CapacitorStorage.setItem(key, jsonData)

      // Fallback storage (localStorage)
      if (typeof window !== "undefined") {
        localStorage.setItem(key, jsonData)
      }

      // Create security backup
      await CapacitorStorage.createSecurityBackup(key, jsonData)

      // Auto backup every 10 operations
      const operationCount = Number.parseInt(localStorage.getItem("operation-count") || "0") + 1
      localStorage.setItem("operation-count", operationCount.toString())

      if (operationCount % 10 === 0) {
        await CapacitorStorage.createAutoBackup()
      }
    } catch (error) {
      console.error(`Error saving ${key}:`, error)
      // Try fallback methods
      if (typeof window !== "undefined") {
        try {
          localStorage.setItem(key, JSON.stringify(data))
        } catch (fallbackError) {
          console.error("Fallback storage also failed:", fallbackError)
        }
      }
    }
  }

  // Effect for initial data load and subscribing to sync events
  useEffect(() => {
    loadData()

    const handleStorageChange = () => {
      loadData()
    }

    const handleVisibilityChange = () => {
      if (!document.hidden) {
        // App became visible, check for sync
        checkCloudSync()
      }
    }

    const handleOnlineStatus = () => {
      if (navigator.onLine) {
        checkCloudSync()
      } else {
        setSyncStatus("offline")
      }
    }

    // Listen for the custom event to re-load data
    habitEventTarget.addEventListener(HABIT_STORAGE_EVENT, handleStorageChange)
    document.addEventListener("visibilitychange", handleVisibilityChange)
    window.addEventListener("online", handleOnlineStatus)
    window.addEventListener("offline", handleOnlineStatus)

    // Cleanup listeners on component unmount
    return () => {
      habitEventTarget.removeEventListener(HABIT_STORAGE_EVENT, handleStorageChange)
      document.removeEventListener("visibilitychange", handleVisibilityChange)
      window.removeEventListener("online", handleOnlineStatus)
      window.removeEventListener("offline", handleOnlineStatus)
    }
  }, [loadData])

  // --- Enhanced Modifier Functions ---
  const addHabit = useCallback(async (habit: Omit<Habit, "id" | "createdAt">) => {
    try {
      const currentHabitsStr = await CapacitorStorage.getItem("habits")
      const currentHabits = currentHabitsStr ? JSON.parse(currentHabitsStr) : []

      const newHabit: Habit = {
        ...habit,
        id: Date.now().toString(),
        createdAt: new Date(),
      }

      const newHabits = [...currentHabits, newHabit]
      await saveDataSafely("habits", newHabits)

      console.log("Habit added successfully:", newHabit)
      dispatchStorageChangeEvent()
    } catch (error) {
      console.error("Error adding habit:", error)
    }
  }, [])

  const updateHabit = useCallback(async (id: string, updates: Partial<Habit>) => {
    try {
      const currentHabitsStr = await CapacitorStorage.getItem("habits")
      const currentHabits = currentHabitsStr ? JSON.parse(currentHabitsStr) : []

      const newHabits = currentHabits.map((h: Habit) => (h.id === id ? { ...h, ...updates } : h))
      await saveDataSafely("habits", newHabits)

      dispatchStorageChangeEvent()
    } catch (error) {
      console.error("Error updating habit:", error)
    }
  }, [])

  const deleteHabit = useCallback(async (id: string) => {
    try {
      const [currentHabitsStr, currentCompletionsStr] = await Promise.all([
        CapacitorStorage.getItem("habits"),
        CapacitorStorage.getItem("completions"),
      ])

      const currentHabits = currentHabitsStr ? JSON.parse(currentHabitsStr) : []
      const currentCompletions = currentCompletionsStr ? JSON.parse(currentCompletionsStr) : []

      const newHabits = currentHabits.filter((h: Habit) => h.id !== id)
      const newCompletions = currentCompletions.filter((c: HabitCompletion) => c.habitId !== id)

      await Promise.all([saveDataSafely("habits", newHabits), saveDataSafely("completions", newCompletions)])

      dispatchStorageChangeEvent()
    } catch (error) {
      console.error("Error deleting habit:", error)
    }
  }, [])

  const reorderHabits = useCallback(async (newHabits: Habit[]) => {
    try {
      await saveDataSafely("habits", newHabits)
      dispatchStorageChangeEvent()
    } catch (error) {
      console.error("Error reordering habits:", error)
    }
  }, [])

  const toggleHabitCompletion = useCallback(async (habitId: string, date: string) => {
    try {
      const currentCompletionsStr = await CapacitorStorage.getItem("completions")
      const currentCompletions = currentCompletionsStr ? JSON.parse(currentCompletionsStr) : []

      const existingIndex = currentCompletions.findIndex(
        (c: HabitCompletion) => c.habitId === habitId && c.date === date,
      )

      if (existingIndex > -1) {
        const existing = currentCompletions[existingIndex]
        let newStatus: CompletionStatus
        switch (existing.status) {
          case "incomplete":
            newStatus = "completed"
            break
          case "completed":
            newStatus = "incomplete"
            break
          case "failed":
            newStatus = "incomplete"
            break
          default:
            newStatus = "completed"
        }
        currentCompletions[existingIndex] = { ...existing, status: newStatus }
      } else {
        currentCompletions.push({ habitId, date, status: "completed" })
      }

      await saveDataSafely("completions", currentCompletions)
      dispatchStorageChangeEvent()
    } catch (error) {
      console.error("Error toggling habit completion:", error)
    }
  }, [])

  const setHabitFailed = useCallback(async (habitId: string, date: string) => {
    try {
      const currentCompletionsStr = await CapacitorStorage.getItem("completions")
      const currentCompletions = currentCompletionsStr ? JSON.parse(currentCompletionsStr) : []

      const existingIndex = currentCompletions.findIndex(
        (c: HabitCompletion) => c.habitId === habitId && c.date === date,
      )

      if (existingIndex > -1) {
        currentCompletions[existingIndex] = { ...currentCompletions[existingIndex], status: "failed" }
      } else {
        currentCompletions.push({ habitId, date, status: "failed" })
      }

      await saveDataSafely("completions", currentCompletions)
      dispatchStorageChangeEvent()
    } catch (error) {
      console.error("Error setting habit failed:", error)
    }
  }, [])

  // --- Read-only Functions ---
  const getHabitCompletion = useCallback(
    (habitId: string, date: string): CompletionStatus => {
      const completion = completions.find((c) => c.habitId === habitId && c.date === date)
      return completion?.status || "incomplete"
    },
    [completions],
  )

  const isHabitScheduledForDay = useCallback((habit: Habit, date: Date): boolean => {
    const dayOfWeek = date.getDay()
    return habit.targetDays && habit.targetDays.length > 0 ? habit.targetDays.includes(dayOfWeek) : false
  }, [])

  const calculateStreak = useCallback(
    (habitId: string): number => {
      const today = new Date()
      let streak = 0
      const currentDate = new Date(today)
      let foundGap = false

      for (let i = 0; i < 365; i++) {
        const dateStr = currentDate.toISOString().split("T")[0]
        const status = getHabitCompletion(habitId, dateStr)

        if (status === "completed") {
          if (!foundGap) {
            streak++
          } else {
            break
          }
        } else if (status === "failed") {
          break
        } else if (status === "incomplete") {
          const isToday = currentDate.toDateString() === today.toDateString()
          const isFuture = currentDate > today

          if (!isToday && !isFuture) {
            foundGap = true
          }
        }
        currentDate.setDate(currentDate.getDate() - 1)
      }
      return streak
    },
    [getHabitCompletion],
  )

  const getHabitsWithProgress = useCallback((): HabitWithProgress[] => {
    const today = new Date().toISOString().split("T")[0]
    return habits.map((habit) => {
      const completedToday = getHabitCompletion(habit.id, today) === "completed"
      const streak = calculateStreak(habit.id)
      let completedDays = 0
      for (let i = 0; i < 7; i++) {
        const checkDate = new Date()
        checkDate.setDate(checkDate.getDate() - i)
        const dateStr = checkDate.toISOString().split("T")[0]
        if (getHabitCompletion(habit.id, dateStr) === "completed") {
          completedDays++
        }
      }
      const completionRate = Math.round((completedDays / 7) * 100)
      const getCurrentWeekDates = () => {
        const today = new Date()
        const currentDay = today.getDay()
        const monday = new Date(today)
        monday.setDate(today.getDate() - (currentDay === 0 ? 6 : currentDay - 1))
        const weekDates = []
        for (let i = 0; i < 7; i++) {
          const date = new Date(monday)
          date.setDate(monday.getDate() + i)
          weekDates.push(date)
        }
        return weekDates
      }
      const weekDates = getCurrentWeekDates()
      const currentWeekCompleted = weekDates.filter((date) => {
        const dateStr = date.toISOString().split("T")[0]
        return getHabitCompletion(habit.id, dateStr) === "completed"
      }).length
      return {
        ...habit,
        completedToday,
        streak,
        completionRate,
        currentWeekCompleted,
      }
    })
  }, [habits, getHabitCompletion, calculateStreak])

  const getWeekDates = useCallback((weekOffset: number) => {
    const today = new Date()
    const currentDay = today.getDay()
    const monday = new Date(today)
    monday.setDate(today.getDate() - (currentDay === 0 ? 6 : currentDay - 1) + weekOffset * 7)
    const weekDates = []
    for (let i = 0; i < 7; i++) {
      const date = new Date(monday)
      date.setDate(monday.getDate() + i)
      weekDates.push(date)
    }
    return weekDates
  }, [])

  useEffect(() => {
    if (habits.length === 0) return
    const weekDates = getWeekDates(weekOffset)
    const habitsWithProgress = getHabitsWithProgress()
    const newCelebratingHabits = new Set<string>()
    habitsWithProgress.forEach((habit) => {
      const completions = weekDates.reduce((acc, date) => {
        if (getHabitCompletion(habit.id, date.toISOString().split("T")[0]) === "completed") {
          return acc + 1
        }
        return acc
      }, 0)
      if (completions >= habit.weeklyTarget && !celebratingHabits.has(habit.id)) {
        const weekKey = `${habit.id}-${weekOffset}`
        const celebratedThisWeek = localStorage.getItem(`celebrated-${weekKey}`)
        if (!celebratedThisWeek) {
          newCelebratingHabits.add(habit.id)
          localStorage.setItem(`celebrated-${weekKey}`, "true")
        }
      }
    })
    if (newCelebratingHabits.size > 0) {
      setCelebratingHabits(newCelebratingHabits)
      const timeout = setTimeout(() => {
        setCelebratingHabits(new Set())
      }, 2000)
      return () => clearTimeout(timeout)
    }
  }, [weekOffset, habits, getHabitsWithProgress, getWeekDates, getHabitCompletion, celebratingHabits])

  return {
    habits,
    completions,
    isLoading,
    syncStatus,
    getHabitsWithProgress,
    toggleHabitCompletion,
    setHabitFailed,
    getHabitCompletion,
    isHabitScheduledForDay,
    addHabit,
    updateHabit,
    deleteHabit,
    reorderHabits,
    celebratingHabits,
    weekOffset,
    setWeekOffset,
    checkCloudSync,
  }
}
